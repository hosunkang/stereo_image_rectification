...
class CalibrationRectification:
    ...
    def calibrate(self):
        ...
        for i in range(len(images_l)):
            ...
            ret_l, corners_l = cv2.findChessboardCorners(gray_l, (6, 9), None, flags=cv2.CALIB_CB_FILTER_QUADS)
            ret_r, corners_r = cv2.findChessboardCorners(gray_r, (6, 9), None, flags=cv2.CALIB_CB_FILTER_QUADS)
            print(ret_l, ret_r)
            ...
            if ret_l & ret_r:
                ...
                img_drawn_l = cv2.drawChessboardCorners(im_l, (6, 9), corners2_l, ret_l)
                img_drawn_r = cv2.drawChessboardCorners(im_r, (6, 9), corners2_r, ret_r)
                left_root = 'calibration/drawnCorners/left/'
                right_root ='calibration/drawnCorners/right/' 
                if not os.path.isdir(left_root):
                    os.mkdir(left_root)
                if not os.path.isdir(right_root):
                    os.mkdir(right_root)
                cv2.imwrite(left_root + str(i) + ".png", img_drawn_l)
                cv2.imwrite(right_root + str(i) + ".png", img_drawn_r)

        cv2.destroyAllWindows()
        print('Save all chess board corner images!!!!')
        ...
        ret_l, self.mtx_l, self.dist_l, self.rvecs_l, self.tvecs_l = cv2.calibrateCamera(objpoints, imgpoints_l,gray_l.shape[::-1], None, None)
        ret_r, self.mtx_r, self.dist_r, self.rvecs_r, self.tvecs_r = cv2.calibrateCamera(objpoints, imgpoints_r,gray_r.shape[::-1], None, None)
    ...
    def draw_comparison(self, im_l, im_r, name):
        ...
        root = './results/undistortedRectified/aligned/'
        if not os.path.isdir(root):
            os.mkdir(root)

        cv2.imwrite(root + str(name) + '.png', aligned)

    def remap(self, left_image, right_image, name):
        # cv2.imshow('cl',left_image)
        # cv2.imshow('cr',right_image)
        # cv2.waitKey(0)
        self.size = left_image.shape
        # undistorting the images witht the calculated undistortion map
        result_l = np.ndarray(shape=([self.size[0], self.size[1], 3]), dtype=np.uint8)
        ...
        # undistorting the images witht the calculated undistortion map
        result_r = np.ndarray(shape=([self.size[0], self.size[1], 3]), dtype=np.uint8)
        ...
        left_root = './remap/remapped/left/'
        if not os.path.isdir(left_root):
            os.mkdir(left_root)
        right_root = './remap/remapped/right/'
        if not os.path.isdir(right_root):
            os.mkdir(right_root)

        cv2.imwrite(left_root + str(name) + '.png', result_l)
        cv2.imwrite(right_root + str(name) + '.png', result_r)
    ...
    def remap_from_capture(self, count, l_camera_port, r_camera_port):
        ...
        while counter < count:
            ...
            left_root = './remap/captures/left/'
            if not os.path.isdir(left_root):
                os.mkdir(left_root)
            right_root = './remap/captures/right/'
            if not os.path.isdir(right_root):
                os.mkdir(right_root)

            cv2.imwrite(left_root + str(counter) + '.png', left_image)
            cv2.imwrite(right_root + str(counter) + '.png', right_image)
